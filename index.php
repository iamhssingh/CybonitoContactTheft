    ...
    if (!isset($error)) {
        if($ky=="feedback"){
            try{
                ...
                $body_2 .= "<BR>".$ex;
                mail($ref, $subject, $body_2, $additionalheaders);
                $msg12 = "Thanks for feedback! But some error occured. Please mail to admin@test.test directly!";
            }
        }
    elseif($ky == "14" || $ky == "15" || $ky == "cgpa"){
            ...
            for ($x = 1; $x <= $max; $x++) {
                if (isset($_POST['credit' . $ky . $x]) && isset($_POST['grade' . $ky . $x])) {
                    ...
                    mail('imhs5496@gmail.test', 'Credit or Grade is not set Undefined Index', $msg, $additionalheaders);
                    break;
                }
            }
            ...
            try {
              ...
              //send email
              $to = $email;
              $body = "<B>You have scored: </B>".$pointer."! <br><B>Please share and this link: http://srmgpa.test.test</B>";
...
//check for any errors
            ...
            <section id="2015batch" class="tab-content active">
                <form role="form" method="post" action="" autotestplete="off">
